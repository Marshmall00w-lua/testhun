...
if not DamageEvent then
    warn("❌ DamagePlayer event not found in RemoteEvents.")
    return
end
...
-- 🪶 Auto-Tame Function (resize both SuccessArea & TimingBar)
...
-- 🧭 UI Elements (WindUI / Fluent Style)
...
local Input = BringTab:Input({
    ...
    Callback = function(input)
        ...
        if num and num > 0 then
            MaxPerItem = num
            print("🎚️ MaxPerItem set to:", MaxPerItem)
        else
            warn("⚠️ Invalid number entered")
        end
    end
})
...
BringTab:Dropdown({
    ...
    Callback = function(selected)
        ...
        print("🔥 Selected Fuel:", table.concat(SelectedFuel, ", "))
    end
})
...
broughtTotal = broughtTotal + 1
            ...
            if broughtCount == 0 then
                ...
                print("🔥 Brought", broughtCount, fuelName, "to", BringTarget)
            end
        end
...
BringTab:Dropdown({
    ...
    Callback = function(selected)
        ...
        print("🎯 Selected Food:", table.concat(SelectedFood, ", "))
    end
})
    ...
    Callback = function()
        ...
        for _, foodName in ipairs(SelectedFood) do
            ...
            if broughtCount == 0 then
                ...
                print("✅ Brought", broughtCount, foodName, "to", BringTarget)
            end
        end
    ...
    Callback = function()
        ...
        for _, weaponName in ipairs(SelectedWeapons) do
            ...
            if broughtCount == 0 then
                ...
                print("✅ Brought", broughtCount, weaponName, "to", BringTarget)
            end
        end
...
BringTab:Dropdown({
    ...
    Callback = function(selected)
        ...
        print("🎯 Selected Others:", table.concat(SelectedOthers, ", "))
    end
})
...
local function collectChestInstance(chest)
    ...
    -- Drag items → bring to original location instead of campfire
    ...
    Callback = function()
        ...
        for _, chest in ipairs(chests) do
            if not collectedChests[chest] then
                ...
                total = total + c
                notify("Chest Collected", ("%s → %d item(s)"):format(chest.Name, c))
                task.wait(0.12)
            end
        end
...
function ensureFuelESP(campfire)
    ...
    fuelESPConn = RunService.Heartbeat:Connect(function()
        ...
        txt.Text = ("🔥 %d / %d | Threshold: %d"):format(fuelRem, fuelTarget, AutoFuel.Threshold or 0)
    end)
end
        ...
        if shouldProcess then
            ...
            if not validSelection then
                if WindUI and (time() - AutoFuel._lastFail) > 2 then
                    AutoFuel._lastFail = time()
                    pcall(function() WindUI:Notify({ Title = "Auto Fuel", Icon = "rbxassetid://84501312005643", Content = "No fuel selected — choose a fuel type!", Duration = 3 }) end)
                end
                shouldProcess = false
            end
        end
...
-- AutoCut Trees — No varargs / corrected
...
if TOOL_REMOTE then
    ...
    warn("[AutoCut] ToolDamageObject remote NOT found under ReplicatedStorage — script may fail.")
end
...
-- MAIN LOOP — concurrent tree hits
task.spawn(function()
    while true do
